...
export default function LiveClassList() {
  ...
  const classes = user?.liveClasses?.length ? user.liveClasses : [
    {
  ...
  return (
    <Card className="bg-[#11131e] border-[#1e2235] rounded-xl overflow-hidden font-sans">
      ...
      <CardContent className="px-3 pb-4 space-y-3">
        {classes.map((cls, idx) => (
          <div
            key={idx}
            className={`flex items-center justify-between p-3.5 rounded-xl transition-colors ${cls.active ? "bg-[#1e233b]/80 border border-[#2b3a61]" : "bg-[#181a25]/80 border border-transparent"
              }`}
          >
